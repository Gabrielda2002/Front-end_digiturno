...
const ConfigUsuarios = () => {
  const [usuarios, setUsuarios] = useState<any[]>([
    { id: '1', nombre: 'Admin Principal', usuario: 'admin', rol: 'administrador', sede: 'Todas', activo: true },
    { id: '2', nombre: 'Juan Operador', usuario: 'juan', rol: 'operador', sede: 'Sede Principal', activo: true },
    { id: '3', nombre: 'María Supervisor', usuario: 'maria', rol: 'supervisor', sede: 'Sede Norte', activo: true }
  ]);
  const [formData, setFormData] = useState({ id: '', nombre: '', usuario: '', password: '', rol: 'operador', sede: '', activo: true });
  const [isEditing, setIsEditing] = useState(false);
    ...
    if (isEditing) {
      setUsuarios(prev => prev.map(usuario => 
        usuario.id === formData.id ? {
          ...formData,
          sede: sedes.find(s => s.value === formData.sede)?.label || formData.sede
        } : usuario
      ));
    } else {
  ...
  const handleEdit = (usuario: any) => {
    // Convertir la sede de texto a valor
    const sedeValue = sedes.find(s => s.label === usuario.sede)?.value || 'todas';
  ...
  const resetForm = () => {
    setFormData({ id: '', nombre: '', usuario: '', password: '', rol: 'operador', sede: '', activo: true });
    setIsEditing(false);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="flex items-center">
            <input
              ...
              className="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded"
            />
            <label htmlFor="activoUsuario" className="ml-2 block text-sm text-gray-700">
              Usuario activo
            </label>
          </div>
        </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {usuarios.map((usuario) => (
              <tr key={usuario.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${usuario.activo ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
